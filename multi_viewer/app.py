"""Multi-User Study Materials Web Viewer - Flask Application."""
import os
...
def _get_user_id():
    """Get current user ID or None for anonymous."""
    return current_user.id if current_user.is_authenticated else None


def get_progress(lang: str, topic: str) -> dict:
    """Get reading progress for a topic. Returns zeros for anonymous users."""
    lessons = get_lessons(lang, topic)
    total = len(lessons)
    user_id = _get_user_id()
    if user_id:
        read_count = LessonRead.query.filter_by(user_id=user_id, language=lang, topic=topic).count()
    else:
        read_count = 0
    return {
        "total": total,
...
def is_lesson_read(lang: str, topic: str, filename: str) -> bool:
    ...
    if not user_id:
        return False
    return LessonRead.query.filter_by(user_id=user_id, language=lang, topic=topic, filename=filename).first() is not None
...
def is_bookmarked(lang: str, topic: str, filename: str) -> bool:
    ...
    if not user_id:
        return False
    return Bookmark.query.filter_by(user_id=user_id, language=lang, topic=topic, filename=filename).first() is not None
...
def index(lang: str):
    ...
    if user_id:
        total_read = LessonRead.query.filter_by(user_id=user_id, language=lang).count()
    else:
        total_read = 0
    overall = {
        "total": total_lessons,
...
def dashboard(lang: str):
    """Dashboard page - overall progress (login required)."""
    topics = get_topics(lang)
    for topic in topics:
        topic["progress"] = get_progress(lang, topic["name"])

    total_lessons = sum(t["lesson_count"] for t in topics)
    total_read = LessonRead.query.filter_by(user_id=current_user.id, language=lang).count()
    overall = {
        "total": total_lessons,
        "read": total_read,
        "percentage": round(total_read / total_lessons * 100) if total_lessons > 0 else 0,
...
def bookmarks(lang: str):
    """Bookmarks page (login required)."""
    bookmarked = Bookmark.query.filter_by(user_id=current_user.id, language=lang) \
...
@login_required
def api_mark_read():
    """Mark a lesson as read/unread for the current user."""
    ...
    existing = LessonRead.query.filter_by(
        user_id=current_user.id, language=lang, topic=topic, filename=filename
    ).first()

    if is_read and not existing:
        lesson_read = LessonRead(user_id=current_user.id, language=lang, topic=topic, filename=filename)
...
@login_required
def api_bookmark():
    """Add or remove a bookmark for the current user."""
    data = request.get_json()
    lang = data.get("lang", DEFAULT_LANG)
    topic = data.get("topic")
    filename = data.get("filename")
    ...
    if existing:
        ...
        bookmark = Bookmark(user_id=current_user.id, language=lang, topic=topic, filename=filename)
...
def api_clear_user_data():
    """Delete all reading history and bookmarks for the current user."""
    deleted_reads = LessonRead.query.filter_by(user_id=current_user.id).delete()
    deleted_bookmarks = Bookmark.query.filter_by(user_id=current_user.id).delete()
    db.session.commit()
    return jsonify({
        "success": True,
...
def api_search():
    """Search API endpoint (no login required)."""
    query = request.args.get("q", "")
...
def api_set_language():
    """Set preferred language (no login required)."""
    data = request.get_json()
    lang = data.get("lang", DEFAULT_LANG)
